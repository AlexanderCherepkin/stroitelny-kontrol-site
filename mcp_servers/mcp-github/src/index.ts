...
async function handlePrList(args: Record<string, unknown>) {
...
async function handlePrGet(args: Record<string, unknown>) {
  const owner = String(args.owner);
  const repo = String(args.repo);
...
async function handlePrDiff(args: Record<string, unknown>) {
  const owner = String(args.owner);
  const repo = String(args.repo);
  const num = Number(args.pull_number);
  const data = await githubFetch(`/repos/${owner}/${repo}/pulls/${num}`, {
...
async function handlePrReview(args: Record<string, unknown>) {
  const owner = String(args.owner);
  const repo = String(args.repo);
  const num = Number(args.pull_number);
  const body = {
...
async function handlePrComments(args: Record<string, unknown>) {
  const owner = String(args.owner);
  const repo = String(args.repo);
  const num = Number(args.pull_number);
...
async function handlePrCreateComment(args: Record<string, unknown>) {
  const owner = String(args.owner);
  const repo = String(args.repo);
  const num = Number(args.pull_number);
  const body = {
...
async function handleIssueList(args: Record<string, unknown>) {
  const owner = String(args.owner);
  const repo = String(args.repo);
  const state = String(args.state || "open");
  const per_page = Number(args.per_page || 30);
