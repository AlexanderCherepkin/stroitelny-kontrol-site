...
function validateCommand(command: string): void {
  ...
  for (const pattern of DANGEROUS_PATTERNS) {
    if (pattern.test(lower)) {
      throw new Error(
        `Command blocked by security policy: matches dangerous pattern "${pattern.source}"`
      );
    }
  }
}
...
const EXECUTE_COMMAND_TOOL: Tool = {
  ...
  description:
    "Execute a shell command within the allowed base directory. " +
...
function handleExecuteCommand(args: Record<string, unknown>) {
