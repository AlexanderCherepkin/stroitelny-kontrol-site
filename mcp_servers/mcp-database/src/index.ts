...
function getConnection(dbName: string): DatabaseSync {
...
const EXECUTE_TOOL: Tool = {
  name: "execute",
  description:
    "Execute a non-SELECT SQL statement (INSERT, UPDATE, DELETE, CREATE, DROP, ALTER). " +
...
const TOOLS: Tool[] = [QUERY_TOOL, EXECUTE_TOOL, TRANSACTION_TOOL, LIST_TABLES_TOOL];

async function handleQuery(args: Record<string, unknown>) {
  await ensureDbDir();
...
async function handleTransaction(args: Record<string, unknown>) {
  await ensureDbDir();
  const dbName = String(args.database);
  ...
  const results: { sql: string; changes?: number; lastInsertRowid?: number; rows?: unknown[] }[] = [];
...
async function handleListTables(args: Record<string, unknown>) {
  await ensureDbDir();
  const dbName = String(args.database);
  const db = getConnection(dbName);
