...
const TOOLS: Tool[] = [READ_FILE_TOOL, WRITE_FILE_TOOL, EDIT_FILE_TOOL, ROLLBACK_FILE_TOOL];
...
async function handleReadFile(args: Record<string, unknown>) {
  ...
  try {
    ...
    if (err.code === "EACCES") throw new Error(`Permission denied: ${args.path}`);
    throw new Error(`Failed to read file: ${err.message}`);
  }
}

async function handleWriteFile(args: Record<string, unknown>) {
  const filePath = resolvePath(String(args.path));
...
async function handleEditFile(args: Record<string, unknown>) {
  const filePath = resolvePath(String(args.path));
  ...
  if (!replaceAll && occurrences > 1) {
    throw new Error(
      `old_string found ${occurrences} times in file. Set replace_all=true to replace all occurrences.`
    );
  }
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify(
          { status: "restored", path: args.path, backup: latestBackup },
          null,
          2
        ),
      },
...
const server = new Server(
  { name: "mcp-replace", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
