...
const HTTP_REQUEST_TOOL = {
    name: "http_request",
    description: "Perform an HTTP request (GET, POST, PUT, DELETE, PATCH). " +
...
async function handleHttpRequest(args) {
    ...
    if (!result) {
        throw new Error(`HTTP request failed after ${retries + 1} attempt(s): ${lastError?.message || "Unknown error"}`);
    }
...
const server = new Server({ name: "mcp-web", version: "1.0.0" }, { capabilities: { tools: {} } });
